...
export default function init3DScene(container, user, callbacks, spawnNode = 'default') {
  // --- STATE ---
  ...
  const createPiBlock = (x, z) => {
    ...
    scene.add(group); 
    buildingsList.push({ group, name: "PI / M BLOCK", bHeight: bHeight+10, bDepth: bDepth });
  };
  ...
  gltfLoader.load(modelUrl, (gltf) => {
    ...
    if (spawnNode === 'eblock') {
      player.position.set(0, 0, -60); // E Block vicinity
    } else {
      player.position.set(45, 0, 2);  // Default spawn near gate
    }
    ...
    // Dynamic initial look camera based on spawn location
  ...
  const handleKey = (e, val) => {
    // Don't process movement keys when user is typing in an input/textarea/chat
    const tag = document.activeElement?.tagName;
    if (tag === 'INPUT' || tag === 'TEXTAREA' || document.activeElement?.isContentEditable) {
      // Clear all keys so robot stops if focus moved while a key was held
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (e.key.toLowerCase() === "e") {
      // Don't toggle zones while a modal/overlay is already open — only allow closing
      if (isObjectivesOpen) {
        isObjectivesOpen = false;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(false, null);
        return;
      }
      // Don't fire if user is typing in an input/textarea/button
      const tag = document.activeElement?.tagName;
      if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'BUTTON' || tag === 'IFRAME') return;

      if (player && player.position.distanceTo(new THREE.Vector3(-45, 0, -45)) < 18) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'pi');
      } else if (player && player.position.distanceTo(new THREE.Vector3(0, 0, -85)) < 20) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'eblock');
      } else if (player && player.position.distanceTo(new THREE.Vector3(55, 0, -78)) < 15) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'chai');
      } else if (player && player.position.distanceTo(new THREE.Vector3(55, 0, -92)) < 15) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'juice');
      } else if (player && player.position.distanceTo(new THREE.Vector3(-70, 0, -100)) < 20) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'cricket');
      } else if (player && player.position.distanceTo(new THREE.Vector3(0, 0, -155)) < 25) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'football');
      } else if (player && player.position.distanceTo(new THREE.Vector3(75, 0, -80)) < 15) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'bblock');
      } else if (player && player.position.distanceTo(new THREE.Vector3(-80, 0, -135)) < 15) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'hblock');
      } else if (player && player.position.distanceTo(new THREE.Vector3(-55, 0, -185)) < 18) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'gblock');
      } else if (player && player.position.distanceTo(new THREE.Vector3(90, 0, -185)) < 20) {
        isObjectivesOpen = true;
        if (callbacks && callbacks.onObjToggle) callbacks.onObjToggle(true, 'gallery');
      }
    }
  });
  ...
  const animate = () => {
    ...
    if (player) {
      ...
      const isMovementBlocked = store.isPaused || store.canInteract || store.activePuzzle;
      ...
      if (!scene.userData.staticColliders) {
          ...
          const collectColliders = (obj) => {
              ...
              if (obj.isMesh) {
                  ...
                  if (box.min.y <= 4 && box.max.y > 0.5) scene.userData.staticColliders.push(box);
              }
              if (obj.children) obj.children.forEach(collectColliders);
          };
          collectColliders(fwLeft); collectColliders(fwRight);
          collectColliders(leftWall); collectColliders(rightWall);
          collectColliders(backWall); collectColliders(gateGroup);
      ...
      if (collision) player.position.copy(oldPosition);
