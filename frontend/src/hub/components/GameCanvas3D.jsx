...
const GameCanvas3D = ({ user, spawnNode }) => {
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    const callbacks = {
      ...
      onObjToggle: (show, zone) => { 
        setShowObjectives(show ? zone : null); 
        if (!show) { 
          setActiveIframeUrl(null); 
          setActiveInternalView(null); 
        } else if (zone !== 'eblock') {
          // Safety: ensure we clear the escape console if we are opening other building info (like AIMS)
          useGameStore.getState().setCanInteract(false);
        }
      }
    };

    const cleanup = init3DScene(ref.current, user, callbacks, spawnNode);
          ...
          {activeIframeUrl ? (
            ...
            <div style={styles.modalCard}>
              ...
              <ul style={styles.gridContainer}>
                ...
                <li style={{...styles.gridCard, background: '#1e3a8a', borderColor: '#3b82f6'}}>
                  <div style={styles.gridCardBox}>
                    <span style={{color: '#60a5fa'}}>Escape Games</span>
                  </div>
                  <button 
                    onClick={() => { 
                      setShowObjectives(null);
                      // Static import — no dynamic import race condition
                      useGameStore.getState().setCanInteract(true);
                    }} 
                    style={{...styles.taskBtn, background: '#2563eb'}}
                  >
                    Enter Console
                  </button>
                </li>
            ...
            <div style={styles.modalCard}>
              ...
              <ul style={styles.gridContainer}>
                ...
                <li style={styles.gridCard}>
                  ...
                  <button onClick={() => { setActiveIframeUrl("https://aims.iiitdwd.ac.in/aims/login/loginHome"); setCompletedTasks(p => ({...p, aims: true})); }} style={styles.taskBtn}>View</button>
                </li>
              </ul>
              <div style={{ marginTop: '20px', textAlign: 'right' }}>
                <button onClick={() => setShowObjectives(null)} style={styles.closeBtn}>Close</button>
